...
Orquesta, en un solo comando y sin intervencion humana, las 6 etapas del
...
from src import data_generator, explainability, feature_engineering, modeling, optimizer, wrangling
...
STAGES: list[tuple[str, callable]] = [
    ("1/6 Ingesta y generacion sintetica de block model + telemetria", data_generator.main),
    ("2/6 Limpieza: Kalman + imputacion espacial + Isolation Forest", wrangling.main),
    ("3/6 Feature engineering geometalurgico (mineralogia, interacciones, wavelets)", feature_engineering.main),
    ("4/6 Entrenamiento multi-output (XGBoost + CatBoost) con walk-forward CV", modeling.main),
    ("5/6 Motor de optimizacion prescriptiva (Algoritmo Genetico)", optimizer.main),
    ("6/6 Explicabilidad SHAP + dashboard + reporte bilingue (ES/EN)", explainability.main),
]
